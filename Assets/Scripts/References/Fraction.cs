...
namespace Mehroz
{
    ...
    public struct Fraction : IComparable, IFormattable
    {
        ...
        /// <remarks>Will reduce the fraction to smallest possible denominator</remarks>
        public Fraction(long numerator, long denominator, bool shouldReduceFraction = true)
        {
			...
			m_shouldReduce = shouldReduceFraction;
            if (shouldReduceFraction) ReduceFraction(ref this);
        }
		...
		/// <summary>
		/// Whether this fraction should be reduced when created with a constructor
		/// </summary>
		/// <example>If true, 2/4 will be reduced to 1/2</example>
		public bool shouldReduceFraction
		{
			get
			{
				return m_shouldReduce;
			}
			set
			{
				m_shouldReduce = value;
			}
		}
        #endregion
